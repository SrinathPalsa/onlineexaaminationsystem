...
public class AQuesa extends HttpServlet {
	public void doGet(HttpServletRequest req, HttpServletResponse res)
		...
		try {
			...
			out.println("<body background='http://localhost:8089/paper_bg.gif'><form method=post name=fr action='http://localhost:8089/servlet/Qadd'>");
			out.println("<font size=4 color=blue><b>");
			ResultSet rs = st
					.executeQuery("select max(quesno) from mquestions");
			if (rs != null) {
				rs.next();
			}
			...
			out.println("<input type=text name=number size=4 value='"
					+ (number + 1) + "'><center><br>");
